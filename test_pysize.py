...
class TestPysize(unittest.TestCase):
    def test_empty_list(self):
        empty_list = []
        self.assertEqual(sys.getsizeof(empty_list), pysize.get_size(empty_list))
    ...
    def test_list_of_collections(self):
        ...
        expected_size = (
            empty_list_size * 2 + empty_tuple_size + empty_dict_size + pointer_byte_size
        )

        self.assertEqual(expected_size, pysize.get_size(collection_list))
    ...
    def test_namedtuple(self):
        ...
        self.assertEqual(
            pysize.get_size(point),
            sys.getsizeof(point) + sys.getsizeof(3) + sys.getsizeof(4),
        )
    ...
    def test_slots(self):
        ...
        self.assertEqual(
            pysize.get_size(s2), pysize.get_size(s1) + 28 + 4 + version_addition
        )
        self.assertEqual(
            pysize.get_size(s3), pysize.get_size(s2) + 28 + 4 + version_addition
        )
        self.assertEqual(
            pysize.get_size(s3), pysize.get_size(s1) + 56 + 8 + version_addition * 2
